...
upgrade_code = '{FCB47F0B-A4B5-3F7C-B7B5-00579BFB2BE5}'
...
program_files_dir = 'ProgramFiles64Folder' if distutils.util.get_platform() == 'win-amd64' else 'ProgramFilesFolder'

build_exe_options = {
    # 'packages': ['os'],
    # 'excludes': [''],
    # 'includes': [''],
    'include_files': ['inc_dir/', 'main.py'],
    # 'include_msvcr': True,  # Since it uses PySide, it cannot be started unless Microsoft's C runtime is included.
    'zip_include_packages': '*',
    'zip_exclude_packages': 'tkinterdnd2',
}
